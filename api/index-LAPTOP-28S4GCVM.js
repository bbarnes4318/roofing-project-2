...
const transformProjectForFrontend = (project) => {
  ...
  return {
    ...
    // Project Manager - Handle both object and string formats
    ...
    phase: project.workflow?.steps?.length > 0 ? 
      project.workflow.steps[project.workflow.currentStepIndex || 0]?.phase || mapStatusToPhase(project.status) :
...
const mapDatabasePhaseToFrontend = (phase) => {
  // Return the database enum value directly since frontend now expects uppercase
  const phaseMap = {
    'LEAD': 'LEAD',
    'PROSPECT': 'PROSPECT',
    'APPROVED': 'APPROVED',
    ...
    const projects = await prisma.project.findMany({
      ...
      include: {
        ...
        workflow: {
          include: {
            steps: {
              take: 3 // Get first 3 steps regardless of completion status
            }
          }
        }
      }
...
app.get('/api/workflows/project/:projectId', async (req, res) => {
...
// @desc    Update workflow step (used by frontend checkboxes)
// @route   PUT /api/workflows/project/:projectId/workflow/:stepId
// @access  Private
app.put('/api/workflows/project/:projectId/workflow/:stepId', async (req, res) => {
  ...
  try {
    ...
    // If completing the step, use WorkflowAlertService to trigger alerts for next step
    if (completed && !step.isCompleted) {
      const WorkflowAlertService = require('../server/services/WorkflowAlertService');
      ...
      if (!completionResult.success) {
        return res.status(500).json({
          success: false,
          message: completionResult.error || 'Failed to complete workflow step'
        });
      }
      
      sendSuccess(res, {
        step: completionResult.workflow.steps.find(s => s.stepId === step.stepId),
        workflow: completionResult.workflow,
        nextStep: completionResult.nextStep
      }, 'Workflow step completed successfully');
      
    } else {
      // Just update the step status without triggering alerts
...
app.post('/api/workflows/:workflowId/steps/:stepId/complete', async (req, res) => {
  ...
  try {
    ...
    // Use WorkflowAlertService to complete the step (this will trigger alerts for next step)
    const WorkflowAlertService = require('../server/services/WorkflowAlertService');
    const completionResult = await WorkflowAlertService.completeWorkflowStep(
      workflowId,
      step.stepId, // Use stepId, not the database ID
      req.user.id, // Use the authenticated user
      notes
    );
    ...
    // Mark alert as resolved if alertId is provided
    if (alertId) {
      try {
        ...
        console.log(`⚠️ Could not mark alert ${alertId} as resolved:`, alertError.message);
      }
    }
    ...
    sendSuccess(res, {
      ...
      workflow: { 
        id: workflowId, 
        overallProgress: completionResult.workflow.overallProgress 
      },
      nextStep: completionResult.nextStep,
      newProgress: completionResult.workflow.overallProgress
    }, 'Workflow step completed successfully');
...
app.post('/api/workflows/:workflowId/steps/:stepId/subtasks/:subTaskId/complete', async (req, res) => {
  ...
  try {
    ...
    // Use WorkflowAlertService to complete the subtask (this will auto-complete step if all subtasks done)
    const WorkflowAlertService = require('../server/services/WorkflowAlertService');
    const completionResult = await WorkflowAlertService.completeSubTask(
      workflowId,
      subTask.step.stepId, // Use stepId, not the database ID
      subTask.subTaskId, // Use subTaskId, not the database ID
      req.user.id, // Use the authenticated user
      notes
    );
    ...
    if (!completionResult.success) {
      return res.status(500).json({
        success: false,
        message: completionResult.error || 'Failed to complete subtask'
      });
    }
    
    sendSuccess(res, { 
      subTask: completionResult.subTask,
      stepCompleted: completionResult.stepCompleted,
      nextStep: completionResult.nextStep
