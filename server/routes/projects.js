      ...
      if (!projectsRoot) {
        ...
        projectsRoot = await prisma.companyAsset.create({
          data: {
            title: 'Projects',
            folderName: 'Projects',
            description: 'Root folder for all project-related documents and resources',
            type: 'FOLDER',
            section: 'PROJECT_MANAGEMENT_DOCUMENTS',
            parentId: null,
            uploadedById: systemUserId,
            accessLevel: 'private',
            path: 'Projects',
            sortOrder: 0,
            tags: ['projects', 'root', 'folder'],
            metadata: {
              icon: 'folder',
              color: '#3B82F6',
      ...
      if (!existingFolder) {
        ...
        const projectFolder = await prisma.companyAsset.create({
          data: {
            ...
            description: `Project folder for ${project.projectName || 'Project'} - ${primaryContact}`,
            type: 'FOLDER',
            section: 'PROJECT_MANAGEMENT_DOCUMENTS',
            parentId: projectsRoot.id,
            uploadedById: systemUserId,
            ...
            sortOrder: 0,
            tags: ['project', 'folder', project.projectNumber?.toString() || ''],
            metadata: {
