...
// @desc    Update other user's workflow assignment (admin only)
// @route   PUT /api/auth/workflow-assignment/:userId
// @access  Private (Admin)
router.put('/workflow-assignment/:userId', authenticateToken, [
  body('workflowAssignment')
    .isIn(['OFFICE', 'ADMIN', 'PROJECT_MANAGER', 'FIELD_CREW', 'ROOF_SUPERVISOR', 'FIELD_DIRECTOR'])
    .withMessage('Invalid workflow assignment')
], asyncHandler(async (req, res, next) => {
  // Check for validation errors
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
  ...
  const { workflowAssignment } = req.body;

  console.log('Auth Debug - Current user role:', req.user.role);
  console.log('Auth Debug - Role check:', req.user.role !== 'ADMIN');

  // Check if current user is admin (you can add more role checks here)
  if (req.user.role !== 'ADMIN') {
    console.log('Auth Debug - Authorization failed: User role is not ADMIN');
    return next(new AppError('Unauthorized to update other users', 403));
  }

  console.log('Auth Debug - Authorization passed, updating user:', userId);

  // Update other user's workflow assignment
  ...
  sendSuccess(res, 200, { user: updatedUser }, 'User workflow assignment updated successfully');
}));

// @desc    Update user workflow assignment
// @route   PUT /api/auth/workflow-assignment
// @access  Private
router.put('/workflow-assignment', authenticateToken, [
  body('workflowAssignment')
    .isIn(['OFFICE', 'ADMIN', 'PROJECT_MANAGER', 'FIELD_CREW', 'ROOF_SUPERVISOR', 'FIELD_DIRECTOR'])
    .withMessage('Invalid workflow assignment')
], asyncHandler(async (req, res, next) => {
  ...
  // Update user's workflow assignment
  const updatedUser = await prisma.user.update({
    where: { id: req.user.id },
    data: { workflowAssignment },
    select: {
  ...
  sendSuccess(res, 200, { user: updatedUser }, 'Workflow assignment updated successfully');
}));
