...
const projectValidation = [
  ...
  body('projectNumber')
    .optional()
    .isNumeric()
    .withMessage('Project number must be a number'),
  body('contacts')
    .isArray({ min: 1, max: 3 })
    .withMessage('Contacts must be an array with at least one and up to three contacts'),
  body('contacts.*.name')
    .isString()
  ...
  body('contacts.*.isPrimary')
    .isBoolean()
    .withMessage('Contact isPrimary must be a boolean')
];
...
router.post('/', projectValidation, asyncHandler(async (req, res, next) => {
  ...
  if (Array.isArray(req.body.contacts)) {
    const primaryCount = req.body.contacts.filter(c => c.isPrimary).length;
    if (primaryCount !== 1) {
      return res.status(400).json({
        success: false,
        message: 'Exactly one contact must be marked as primary',
        errors: [{ param: 'contacts', msg: 'Exactly one contact must be marked as primary' }]
      });
    }
  }
