...
class ProjectService {
  ...
  async getProjectWorkflow(projectId) {
    try {
      ...
      if (!workflow) {
        ...
        if (project) {
          return await this.createDefaultWorkflow(projectId, project.projectType);
        }
  ...
  async createProject(projectData, userId) {
    try {
      ...
      // **CRITICAL: Create complete workflow with all 27 steps**
      await this.createDefaultWorkflow(project.id, project.projectType, userId);
    ...
    try {
      ...
      workflow.steps.forEach(step => {
        if (step.subTasks && step.subTasks.length > 0) {
          totalSubTasks += step.subTasks.length;
          completedSubTasks += step.subTasks.filter(subTask => subTask.isCompleted).length;
        }
      });
      ...
      console.log(`Updated workflow progress: ${completedSubTasks}/${totalSubTasks} = ${progress}%`);
  ...
  async createDefaultWorkflow(projectId, projectType, userId = null) {
    try {
      console.log(`Creating default workflow for project ${projectId} (${projectType})`);
      ...
      // Create workflow with all 27 steps (same as seed file structure)
      const workflow = await prisma.projectWorkflow.create({
        data: {
          ...
          createdById: userId,
          // Create all 27 workflow steps - This preserves ALL project phases, sections, and line items
          steps: {
            create: this.getDefaultWorkflowSteps()
          }
        },
        include: {
          steps: {
      ...
      console.log(`✅ Created default workflow with ${workflow.steps.length} steps`);
      return workflow;
    } catch (error) {
  ...
  // **CRITICAL: All 27 default workflow steps preserving exact phases, sections, and line items**
