...
const CustomersPage = ({ colorMode }) => {
  ...
  useEffect(() => {
    if (!searchTerm) {
      ...
      const filtered = customers.filter(customer =>
        customer.primaryName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        customer.primaryEmail.toLowerCase().includes(searchTerm.toLowerCase()) ||
        customer.primaryPhone.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleAddCustomer = async (e) => {
    ...
    try {
      ...
      setNewCustomer({ primaryName: '', primaryEmail: '', primaryPhone: '', address: '' });
  ...
  const handleEditCustomer = async (e) => {
    ...
    try {
      setError('');
      const response = await customersService.update(selectedCustomer.id, selectedCustomer);
      setCustomers(prev => prev.map(c => c.id === selectedCustomer.id ? response.data.customer : c));
  ...
  const renderCustomerCard = (customer) => {
    ...
    return (
      <div key={customer.id} className={`${colorMode ? 'bg-slate-800/90 hover:bg-slate-700/90 border-slate-600/50' : 'bg-white hover:bg-gray-50 border-gray-200'} rounded-lg shadow-sm border transition-all duration-200 hover:shadow-md overflow-hidden`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <a 
                        href={`tel:${customer.primaryPhone?.replace(/[^\d+]/g, '') || ''}`}
    ...
    >
      <form onSubmit={handleAddCustomer} className="space-y-2">
        ...
        <div>
          ...
          <input
            type="text"
            value={newCustomer.primaryName}
            onChange={(e) => setNewCustomer({ ...newCustomer, primaryName: e.target.value })}
    ...
    >
      <form onSubmit={handleEditCustomer} className="space-y-2">
        ...
        <div>
          ...
          <input
            type="text"
            value={selectedCustomer?.primaryName || ''}
            onChange={(e) => setSelectedCustomer({ ...selectedCustomer, primaryName: e.target.value })}
            className={`w-full p-1 border rounded text-xs transition-colors focus:ring-1 focus:ring-blue-500 focus:border-blue-500 ${
              colorMode
                ? 'bg-slate-700 border-slate-600 text-white placeholder-gray-400'
                : 'bg-white border-gray-300 placeholder-gray-500'
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setSelectedCustomer({ ...selectedCustomer, primaryEmail: e.target.value })}
            className={`w-full p-1 border rounded text-xs transition-colors focus:ring-1 focus:ring-blue-500 focus:border-blue-500 ${
              colorMode
                ? 'bg-slate-700 border-slate-600 text-white placeholder-gray-400'
                : 'bg-white border-gray-300 placeholder-gray-500'
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setSelectedCustomer({ ...selectedCustomer, primaryPhone: e.target.value })}
            className={`w-full p-1 border rounded text-xs transition-colors focus:ring-1 focus:ring-blue-500 focus:border-blue-500 ${
              colorMode
                ? 'bg-slate-700 border-slate-600 text-white placeholder-gray-400'
                : 'bg-white border-gray-300 placeholder-gray-500'
    ...
    >
      <div className="space-y-2">
        <p className={`text-xs ${colorMode ? 'text-gray-300' : 'text-gray-600'}`}>
          Are you sure you want to delete <strong>{customerToDelete?.primaryName}</strong>? This action cannot be undone.
        </p>
