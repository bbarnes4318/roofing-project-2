...
const ProjectsPage = ({ onProjectSelect, onProjectActionSelect, onCreateProject, projects, colorMode, projectSourceSection, onNavigateBack, scrollToProject }) => {
    ...
    const getPhaseText = (status) => {
        ...
        // Map project status to the 6 correct phases using database enum values
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                                <span className={`text-[8px] font-bold ${colorMode ? 'text-blue-300' : 'text-blue-600'}`}>
                                    #{project.projectNumber || project.id}
                                </span>
                                <h3 className={`text-sm font-semibold ${colorMode ? 'text-white' : 'text-gray-900'} truncate`}>
                                    {project.projectName}
                                </h3>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-medium ${
                                    (project.phase || getPhaseText(project.status)) === 'LEAD' ? 'bg-blue-100 text-blue-800' :
                                    (project.phase || getPhaseText(project.status)) === 'PROSPECT' ? 'bg-teal-100 text-teal-800' :
                                    (project.phase || getPhaseText(project.status)) === 'APPROVED' ? 'bg-purple-100 text-purple-800' :
                                    (project.phase || getPhaseText(project.status)) === 'EXECUTION' ? 'bg-orange-100 text-orange-800' :
                                    (project.phase || getPhaseText(project.status)) === 'SUPPLEMENT' ? 'bg-pink-100 text-pink-800' :
                                    (project.phase || getPhaseText(project.status)) === 'COMPLETION' ? 'bg-green-100 text-green-800' :
                                    'bg-blue-100 text-blue-800'
                                }`}>
                                    {getPhaseDisplayName(project.phase || getPhaseText(project.status))}
                                </span>
                            ...
                            <div className="flex-1">
                                ...
                                <div className="space-y-0.5">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <a 
                                            href={`tel:${project.customer?.primaryPhone || '(555) 000-0000'}`}
                                            className={`text-[9px] text-blue-600 hover:text-blue-800 hover:underline transition-colors`}
                                        >
                                            {project.customer?.primaryPhone || '(555) 000-0000'}
                                        </a>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <span className="text-[10px]">✉️</span>
                                        <a 
                                            href={`mailto:${project.customer?.primaryEmail || 'customer@email.com'}`}
                                            className={`text-[9px] text-blue-600 hover:text-blue-800 hover:underline transition-colors`}
                                        >
                                            {project.customer?.primaryEmail || 'customer@email.com'}
                                        </a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className={`p-2 rounded-lg transition-all duration-200 ${colorMode ? 'bg-slate-700/20 border border-slate-600/30' : 'bg-gray-50/90 border border-gray-200/50'} ${expandedProgress[`${project.id}-materials-labor`] ? (colorMode ? 'border-4 border-blue-400' : 'border-4 border-blue-500') : ''}`}>
                        ...
                        >
                            <div className="flex items-center justify-between mb-0.5">
                                ...
                                <div className="flex items-center gap-1">
                                    <span className={`text-[8px] font-bold ${colorMode ? 'text-white' : 'text-gray-800'}`}>{WorkflowProgressService.calculateProjectProgress(project).overall}%</span>
                                    <svg 
                        ...
                        {expandedProgress[`${project.id}-materials-labor`] && (
                            <div className="space-y-1.5 mt-1.5">
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-0.5">
                                        <span className={`text-xs font-semibold ${colorMode ? 'text-gray-300' : 'text-gray-600'}`}>Materials</span>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-0.5">
                                        <span className={`text-xs font-semibold ${colorMode ? 'text-gray-300' : 'text-gray-600'}`}>Labor</span>
                                        <span className={`text-xs font-bold ${colorMode ? 'text-white' : 'text-gray-800'}`}>{Math.round(projectTrades.reduce((sum, trade) => sum + trade.laborProgress, 0) / projectTrades.length)}%</span>
                                    </div>
                                    <div className={`w-full h-2 bg-gray-200 rounded-full overflow-hidden ${colorMode ? 'bg-slate-600' : 'bg-gray-200'}`}>
                                        <div 
                        ...
                        {expandedProgress[`${project.id}-materials-labor`] && (
                            <div>
                                ...
                                >
                                    <span className={`text-xs font-semibold ${colorMode ? 'text-gray-300' : 'text-gray-600'}`}>Trades</span>
                                    <svg 
