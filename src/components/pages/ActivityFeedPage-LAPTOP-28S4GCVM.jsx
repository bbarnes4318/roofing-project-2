...
const ActivityFeedPage = ({ activities, projects, onProjectSelect, onAddActivity, colorMode }) => {
    ...
    const handlePost = () => {
        ...
        setShowAddPost(false); // Close the add post section after posting
    };
            ...
            {tab === 'feed' && (
              <>
                <div className={`${colorMode ? 'bg-[#0f172a] text-white' : 'bg-white text-gray-800'} shadow-[0_2px_8px_rgba(0,0,0,0.1)] rounded-[8px] px-4 py-3 mb-4`}>
                    ...
                    {/* Add POST Toggle Button */}
                    <div className="mb-3">
                        <button
                            onClick={() => setShowAddPost(!showAddPost)}
                            className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 ${
                                showAddPost 
                                    ? (colorMode ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white')
                                    : (colorMode ? 'bg-[#1e293b] text-gray-300 hover:bg-[#232b4d]' : 'bg-gray-100 text-gray-700 hover:bg-gray-200')
                            }`}
                        >
                            <span className="text-[8px] font-semibold">
                                {showAddPost ? '−' : '+'} Add POST
                            </span>
                        </button>
                    </div>

                    {/* Collapsible Add Post Section */}
                    {showAddPost && (
                        <div className={`p-3 mb-4 rounded border ${colorMode ? 'bg-[#1e293b] border-[#3b82f6]/30' : 'bg-gray-50 border-gray-200'}`}>
