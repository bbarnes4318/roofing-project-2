// Individual update script for all remaining line items
const lineItemUpdates = [
  // Based on the user's provided list, here are all updates
...
async function performIndividualUpdates() {
  ...
  for (const update of lineItemUpdates) {
    ...
    try {
      console.log(`🔄 Updating: "${update.currentName}" -> "${update.newName}"`);
      
      const response = await axios.put(`http://127.0.0.1:5000/api/workflows/line-items/${lineItem.id}`, {
      ...
      if (response.data.success) {
        console.log(`✅ Updated: ${lineItem.id}`);
        successCount++;
      } else {
        console.log(`❌ Failed: ${response.data.message}`);
        errors.push({ id: lineItem.id, error: response.data.message });
        errorCount++;
      }
      ...
      console.log(`❌ Error updating ${lineItem.id}: ${error.response?.data?.message || error.message}`);
      errors.push({ id: lineItem.id, error: error.message });
      errorCount++;
    }
  }
  ...
  console.log('\n🎉 Individual update process complete!');
}

// Run the individual updates
if (require.main === module) {
  performIndividualUpdates().catch(console.error);
}

module.exports = { performIndividualUpdates, lineItemUpdates };